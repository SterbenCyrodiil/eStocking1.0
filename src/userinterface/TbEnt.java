...
public class TbEnt extends JInternalFrame {
	...
	void tablecarreg(){
		...
		if(cbOrdem.getSelectedIndex()==0){
    		qtm.setQuery("select * from entidade"
    				+ " order by entidade.cod_entidade ASC");
		   
    	}
    	
    	if(cbOrdem.getSelectedIndex()==1){
    		qtm.setQuery("select * from entidade"
    				+ " order by entidade.cod_entidade DESC");
